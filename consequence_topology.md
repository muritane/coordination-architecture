...
## Notes on Execution, Disturbance Propagation, and Persistence in Constrained Systems
...
Consequences are treated not as:
...
but as **runtime effects produced when actions execute through constrained systems**.
...
It maps how execution produces effects under constraint.
...
- an invocation commits to an execution path,
...
- assign blame or moral responsibility,
- claim predictive certainty,
- provide complete control strategies,
- replace domain-specific models or analyses.
...
In many systems, actions appear consequence-free because:
...
Under these conditions, **interpretive evaluation temporarily substitutes for structural accounting**.
...
- it is not erased by reinterpretation,
- it is not nullified by denial.
...
This is an **analogy**, not a physical law.

The operational claim is limited:

> Disturbance deferred at one layer reappears as constraint, cost, or risk at another.

---

## Consequence Topology (Operational)

**Consequence topology** describes the structure governing how executable effects propagate after an invocation.
...
- High fan-out distributes effects across components or actors.
...
### Observability (Signal Attenuation)
...
## Action, Non-Action, and Execution

An **action** is any invocation that causes the system to execute a transition.
...
A non-action executes wherever the system defines a default transition.

Silence, delay, or inaction invokes whatever behavior the interface specifies.
...
- **Execution space** — the transitions the system will execute when invoked.

When decision space collapses to a single option, choice may be minimal or absent, but execution still occurs.
...
regardless of autonomy, intent, or awareness.
...
Buffers must sometimes be reduced:
...
- latency inflation,
- correction overhead,
- loss of controllability margin.

---

## Probabilistic Correction Is Not Free

Retries, redundancy, failover, and averaging mask local failure.
...
Horizons vary by context:

- milliseconds for control loops,
- days for operations,
- years for infrastructure,
- decades for institutions.

Without a declared horizon, viability claims are incomplete.
...
When delayed or non-local consequences surface, observers often infer:
