...
- psychology, cognition, or consciousness,
- intent, motivation, or preference formation,
...
- **Execution** — state-changing action whose consequences propagate through coupled state.
...
## Primitive constraints (non-optional)
...
## Critical separations (must not collapse)
...
- resistance to immediate decoherence under disturbance.

Existence does **not** require intention, optimization, awareness, or recognition.

Evidence of existence is **causal propagation**, not narrative description.
...
- subsidized (external absorption of cost),
- displaced (exported to other subsystems),
- masked (deferred via metrics or narratives).

Apparent absence of pressure often indicates **displacement**, not resolution.
...
Interpretation does not propagate.  
...
- **coupling** — how many subsystems depend on the closure,
- **reversibility cost** — difficulty of reopening distinctions.

Large-extent, high-coupling, high-cost closures are primary sources of brittleness and downstream coercion-like dynamics (structural, not moral).
...
A metric is execution-real only if it:

- retains minimally sufficient degrees of freedom for viability,
- is bounded and computable,
- yields determinate outcomes,
- is substitutable within its regime,
- **fails explicitly** when saturated or misapplied.

Metrics do not encode truth or values.  
They encode what must remain fixed for viability over a horizon.

Values that do not bind compression remain interpretive and execution-inert.
...
- acceptance and refusal are both executable paths,
- consequences are bounded relative to the horizon,
...
- entry was decidable,
- outcomes depend on controllable variables within the closure.

Absent these conditions, responsibility claims are rhetorical rather than structural.
...
Coordination artifacts route pressure; they do not eliminate it.
...
- **Persist** — execution survives disturbance and depletion.
- **Scale** — execution becomes a reusable building block.
...
It produces a new composite topology that may enable existence or persistence while masking fragility via dependency.
...
Redesign authority cannot be fully encoded in settled metrics, because metrics encode the closure being revised.

Redesign authority resides at interruptible boundaries where failure is containable.

Failure to redesign when invariants break is a primary collapse mode.
...
- what survived depletion.

Values are not injected.  
They are residues of abstraction that survived execution.
